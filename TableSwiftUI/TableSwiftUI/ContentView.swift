...
struct DetailView: View {
    ...
    var body: some View {
        ...
        /* this is the code I wwanted to use to grab the weblink from my dataset for each item
         Link("View Official Website",
                 destination: URL(Item: (item.webLink)!)
         code edits recommended by Gemini
         */
            if let url = URL(string: item.webLink) {
                Link("View Official Website", destination: url)
            }
        ...
        //Spacer() gemini says this is redundant due to scrollView
    } // end body
    }
  } // end DetailView
